...
class GUI:
    ...
    def spawn_one_func(self):
        print("Bir clone aktarılıyor. Lütfen canlı ve izleme  artmasını bekleyin.")
    ...
    def delete_one_func(self):
        print("Bir clone çıkartılıyor. Lütfen canlı ve izleme düşmesini bekleyin.")
    ...
    def run(self):
        ...
        root.title(f'MedyaCreative twitch viewer bot | v{version} | UmursamazAdam')
        ...
        # left
        proxy_available_text = tk.Label(root, text="Kullanılabilir Proxyler", borderwidth=2)
        proxy_available_text.place(x=40, y=100)
        proxy_available = tk.Label(root, text="0", borderwidth=2, relief="solid", width=5)
        proxy_available.place(x=70, y=10)

        # right
        instances_text = tk.Label(root, text="Cloneler", borderwidth=2)
        ...
        watching_instances_text = tk.Label(root, text="İzleyiciler", borderwidth=2)
        ...
        channel_url.insert(0, "https://www.twitch.tv/medyacreative")

        spawn_one = tk.Button(
            root, width=15, anchor="w", text='1 Clone Gönder', command=lambda: self.spawn_one_func()
        )
        spawn_one.place(x=180, y=40)
        spawn_three = tk.Button(
            root, width=15, anchor="w", text='3 Clone Gönder', command=lambda: self.spawn_three_func()
        )
        spawn_three.place(x=180, y=80)
        destroy_one = tk.Button(
            root, width=15, anchor="w", text='1 Clone Çıkar', command=lambda: self.delete_one_func()
        )
        destroy_one.place(x=305, y=40)
        destroy_all = tk.Button(
            root, width=15, anchor="w", text='Tüm Cloneleri Çıkar', command=lambda: self.delete_all_func()
        )
        ...
        # bottom
        lbl = tk.Label(root, text=r"MedyaCreative Twitch Canlı Yayın İzleyici Paneli", fg="blue", cursor="hand2")
